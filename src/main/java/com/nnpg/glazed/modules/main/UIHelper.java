...
public class UIHelper extends Module {
    ...
    private void onPacketSend(PacketEvent.Send event) {
        ...
        if (enableAutoAdvance.get()) {
            ...
            if (pkt != null && pkt.getClass().getSimpleName().equals("ClickSlotC2SPacket")) {
                try {
                    java.lang.reflect.Method m = pkt.getClass().getMethod("getStack");
                    Object stack = m.invoke(pkt);
                    if (stack instanceof net.minecraft.item.ItemStack) {
                        net.minecraft.item.ItemStack is = (net.minecraft.item.ItemStack) stack;
                        if (StringUtils.convertUnicodeToAscii(is.getName().getString()).equals("drop loot")) {
