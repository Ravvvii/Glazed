...
public class RandomBetweenDoubleSetting extends Setting<RandomBetweenDouble> {
    ...
    public RandomBetweenDouble load(NbtCompound tag) {
        double min = defaultValue.min;
        double max = defaultValue.max;

        if (tag.contains("min")) {
            java.util.Optional<Double> opt = tag.getDouble("min");
            if (opt != null && opt.isPresent()) min = opt.get();
        }

        if (tag.contains("max")) {
            java.util.Optional<Double> opt = tag.getDouble("max");
            if (opt != null && opt.isPresent()) max = opt.get();
        }
